...
//echo '<pre>';print_r($id_pengajuan_pemohon);echo '</pre>'; exit();         
        ...
        $data['kode_dpsj_edit'] = $result['kode_dpsj'];
        $data['atas_nama'] = $result['deskripsi_dpsj'];
        ...
        // set tahun edit dan bulan edit dari data pertama
        $data['tahun_edit'] = substr($data['tgl_diajukan'], 0, 4);
        $data['bulan_edit'] = substr($data['tgl_diajukan'], 5, 2);
	
        if(!empty($data['rincian'])){
	        $data['tahun_edit'] = $rincian[0]['tahun'];
	        $data['bulan_edit'] = $rincian[0]['bulan'];
        }
    ...
    public function simpan()
    {
        ...
        $tanggal = $this->input->post('tgl_diajukan');
        ...
        foreach ($rincian as $key => $value) {
            ...
            $tahun = $value['tahun'];
            $bulan = $value['bulan'];
            $kode_dpsj = $value['kode_dpsj'];
            $deskripsi_dpsj = $value['deskripsi_dpsj'];
    ...
    public function deletePengajuan(){
        ...
        // delete data pengajuan pada tabel pengajuan_rincian
        $this->db->where('id_pengajuan_pemohon', $id_pengajuan_pemohon);
        $this->db->delete('pengajuan_rincian');

        // delete data pengajuan pada tabel monitoring
        $this->db->where('id_pengajuan_pemohon', $id_pengajuan_pemohon);
        $this->db->delete('monitoring');

        // delete pengajuan pada tabel kendali dokumen
        // cari dulu nomor_pengajuan
        $sql = "SELECT nomor_pengajuan FROM monitoring WHERE id_pengajuan_pemohon = '$id_pengajuan_pemohon'";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        foreach($result as $row){
            $nomor_pengajuan = $row['id_pengajuan_pemohon'];
        }

        // delete data pengajuan pada tabel kendali_dokumen
        $this->db->where('nomor_pengajuan', $nomor_pengajuan);
        $this->db->delete('kendali_dokumen');
