...
class Pengajuan_D02 extends CI_Controller {
    ...
    public function form() {
        ...
        if($data['kode_bidang'] == '014'){
            $sql = "SELECT * FROM pejabat WHERE kd_struktur = 2 AND end_date > date(now()) AND KodeBidang = '$data[kode_bidang]' ORDER BY kd_struktur";
        } else if($data['kode_bidang'] == '022') {
            // jika tanggal hari ini berada di antara tanggal 18 Agustus 2026 dan 2 September 2026 maka set nip = '100111610210100891' => Manajer Pendidikan
            $tgl_hari_ini = date('Y-m-d');
            if($tgl_hari_ini >= '2026-08-18' && $tgl_hari_ini <= '2026-09-02') {
                $sql = "SELECT * FROM pejabat WHERE kd_struktur = 1 AND end_date > date(now()) AND nip = '100111610210100891' ORDER BY kd_struktur";
            } else {
        ...
        //echo '<pre>';$this->session->userdata('logged_anggaran');print_r($sql);print_r($result);echo '</pre>';exit();
        //$data = array();
        ...
        //echo '<pre>';print_r($data_pejabat);echo '</pre>';
        // ambil nama unit
        ...
        $sql = "SELECT * FROM unit_kerja WHERE kode_dpsj IN ($kode_dpsj)";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        //$data = array();
        foreach($result as $row){
        ...
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('template/header', $data);
            $this->load->view('template/tes_menu', array('menu' => menu()) );
            $this->load->view('unit_kerja/form_pengajuan_d02', $data);
            $this->load->view('template/footer');
			$this->load->view('anggaran/pengajuan_script');
			//$this->load->view('unit_kerja/form_pengajuan_d02_script');
        } else {
            // Get kode unit
