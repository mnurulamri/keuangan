...
class Pengajuan_ajax extends CI_Controller {
    ...
    public function data()
    {
        ...
        // modif kode_dpsj
        //$kode_dpsj[0] = $this->session->userdata('logged_anggaran')['kode_dpsj'];
        //$kode_dpsj = implode("','", $kode_dpsj);
        //$kode_dpsj = "'".$kode_dpsj."'";
        //$data['kode_dpsj'] = $kode_dpsj;
        ...
        //$where = "WHERE kode_dpsj IN ($kode_dpsj) AND YEAR(tanggal) = $tahun AND MONTH(tanggal) = '$bulan' ";
		...
		if ($status == 'Diretur'){
			$where .= " AND kode_status IN (12,14,22,32,33,42,43,52,53,62,63,64,65) AND nomor_pengajuan LIKE '%$keywords%'";
		} else if ($status == '1'){
			$where .= " AND kode_status IN (1,10) AND nomor_pengajuan LIKE '%$keywords%'";
		} else if ($status == 'Semua' && $keywords == ''){
			$where .= " AND kode_status >= 0 AND (nomor_pengajuan LIKE '%$keywords%' or nomor_pengajuan is null)";
		} else {
        ...
        */
        ...
        # get records
        $sql = "SELECT a.*, tahun, bulan
                FROM view_pengajuan a
                JOIN view_tahun_bulan b on a.id = b.id_pengajuan_pemohon AND tahun = $tahun AND bulan ='$bulan' $where ORDER BY id DESC $limit";
        $query = $this->db->query($sql); //$query = $this->db->get();
        
        # get total records
        $sql_count = "SELECT a.id
                FROM view_pengajuan a
                JOIN view_tahun_bulan b on a.id = b.id_pengajuan_pemohon AND tahun = $tahun AND bulan ='$bulan' 
                $where 
                ORDER BY id DESC";
