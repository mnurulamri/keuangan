...
class Pengajuan_daftar extends CI_Controller {
    ...
    public function formKonfirmasiAjukan() {
        ...
        $data['tgl_diajukan'] = $result['tgl_diajukan'];
        ...
        // deskripsi dpsj dan kode_dpsj diambil dari tabel pengajuan_pemohon
    ...
    public function ajukan() {
        ...
        // jika pernah diajukan atau status di tolak anggaran maka update
        // cek dulu id_pengajuan_pemohon di tabel monitoring
        $sql = "SELECT id_pengajuan_pemohon, kode_status FROM monitoring WHERE id_pengajuan_pemohon = '$id_pengajuan_pemohon'";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        foreach($result as $row){
        ...
        if($query->num_rows() > 0){
            // jika datanya ada maka update
            // update tabel pengajuan_pemohon dulu
            $data = array(
                'kode_status' => 1, // Set status ke "ajukan ulang"
                'tgl_diajukan' => $tgl_diajukan
            );
            
            // jika kode status tersimpan = 12 (ditolak anggaran) maka baik untuk Form D01 dan D02 maka update ke 10 (ajukan ulang)
            // jika kode status tersimpan = 52 (dikembalikan verifikator) maka update ke 51 (menunggu pemeriksaan verifikator)
            // jika kode status tersimpan = 54 (dikembalikan verifikator) maka update ke 51 (menunggu pemeriksaan verifikator)
            // jika kode status tersimpan = 62 (dipending korpum), maka update ke 51 (menunggu pemeriksaan verifikator)
            if($kode_status == 12){ 
                // jika statusnya ditolak anggaran maka update ke 1 (ajukan ulang)
                $kode_status = 10;
            } else if($kode_status == 52){ 
                // jika status dikembalikan verifikator atau dipending korpum maka update ke 1 (ajukan ulang)
                if($nama_form == 'D01'){
                    $kode_status = 51;  // menunggu pemeriksaan verifikator
                } else if($nama_form == 'D02'){
                    $kode_status = 51;
                }
            } else if($kode_status == 62){
                // jika status dipending korpum maka update ke 61 (menunggu pemeriksaan korpum)
                $kode_status = 61;
            } else if($kode_status == 63){
                // jika status dikembalikan korpum maka update ke 66 
                // proses ajukan ulang tanpa melalui kasir dan manajer dan menuju proses pengisian SPJ)
                $kode_status = 66;  // menunggu pemeriksaan anggaran
            } else if($kode_status == 67){
                // jika status dalam proses pengisian SPJ maka update ke 66
                // proses ajukan ulang tanpa melalui kasir dan manajer dan menuju proses pengisian SPJ)
                $kode_status = 51;  // menunggu pemeriksaan korpum
            } else {
                // jika statusnya selain itu maka update ke 1 (ajukan ulang)
                $kode_status = 1;
            }
            ...
            // update tabel monitorig
            $data = array(
                'kode_status' => $kode_status, // Set status ke "ajukan ulang"
            ...
            // ambil data tahun dan bulan dari tabel pengajuan_rincian untuk persiapan penomoran
            $sql = "SELECT DISTINCT tahun, bulan FROM pengajuan_rincian WHERE id_pengajuan_pemohon = '$id_pengajuan_pemohon'";
            $query = $this->db->query($sql);
            $result = $query->result_array();
            foreach($result as $row){
                $tahun = $row['tahun'];
            ...
            $kode_unit = $this->session->userdata['logged_anggaran']['kode_unit'];
            $nomor_pengajuan = $this->Anggaran_model->generate_nomor_pengajuan($kode_unit, $tahun, $bulan, $nama_form);
