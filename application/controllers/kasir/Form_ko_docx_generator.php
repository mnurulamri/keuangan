...
class Form_ko_docx_generator {
    public function generateDetailedTable($templatePath, $outputPath, $data, $rincianData, $array, $array_tgl) {
        try {
            ...
            foreach ($rincianData as $index => $item) {
                $rowNumber = $index + 1;
                $uraian = htmlspecialchars($item['uraian'], ENT_QUOTES, 'UTF-8');
                ...
                $template->setValue('uraian#' . $rowNumber, $uraian ?? '');
    ...
    private function calculateSummary($rincianData) {
        ...
        $total_pengajuan = 0;
        $total_penarikan = 0;
        foreach ($rincianData as $row) {
            $total_pengajuan += $row['nominal_pengajuan'];
            $total_penarikan += $row['nominal_pengajuan'];
        }
        return [
