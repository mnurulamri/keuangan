...
class Monitoring extends CI_Controller {
    ...
    // retur saat pencairan
    public function retur_pengajuan() {
        $id_monitoring = $this->input->post('id_monitoring');
        $id_pengajuan_pemohon = $this->input->post('id_pengajuan_pemohon');
        $kasir_keterangan = $this->input->post('kasir_keterangan');
        $kasir_tgl_pending = date('Y-m-d H:i:s');
        $username = $this->session->userdata('logged_anggaran')['username'] ?? '';
        $kode_status = '40'; // Set status ke '42' untuk pending kasir
        
        // Update data monitoring
        $data = array(
            'kasir_keterangan_pending' => addslashes($kasir_keterangan),
            'kasir_tgl_pending' => $kasir_tgl_pending,
            'kasir_username' => $username,  // disetuijui oleh
            'kode_status' => $kode_status, // Set status ke '42' untuk pending kasir
        );        
        //print_r($data);
        // update tabel monitoring
        $sql = $this->db->set($data)->where('id', $id_monitoring)->update('monitoring');
    ...
    public function pending() {
        ...
        $kasir_tgl_pending = date('Y-m-d H:i:s');
        $username = $this->session->userdata('logged_anggaran')['username'] ?? '';
        $kode_status = '42'; // Set status ke '42' untuk pending kasir
    ...
    public function batal() {
        ...
        $id_pengajuan_pemohon = $this->input->post('id_pengajuan_pemohon');
        $kasir_keterangan = $this->input->post('kasir_keterangan');
        $kasir_tgl_batal = date('Y-m-d H:i:s');
        $username = $this->session->userdata('logged_anggaran')['username'] ?? '';
        $kode_status = '43'; // Set status ke '43' untuk batal kasir
        
        // Update data monitoring
        $data = array(
            'kasir_keterangan_batal' => addslashes($kasir_keterangan),
            'kasir_tgl_batal' => $kasir_tgl_batal,
            'kasir_username' => $username,  // disetuijui oleh
            'kode_status' => $kode_status, // Set status ke '43' untuk batal kasir
        );        
        //print_r($data);
        // update tabel monitoring
        $sql = $this->db->set($data)->where('id', $id_monitoring)->update('monitoring');
    ...
    public function detailApproval() {
        ...
        $data['nominal_disetujui_umko'] = $monitoring['nominal_disetujui_umko'] ?? '0';
        $data['untuk_pembayaran'] = $monitoring['nomor_pengajuan'].', '.$monitoring['uraian'];
        $data['yang_menyerahkan'] = $monitoring['kasir_username'] ?? '';
    ...
    public function konfirmasiApprovalTerimaSPJ() {	
        // get all data from pengajuan_rincian berdasarkan kode_bidang
        //$kode_bidang = $this->session->userdata('kode_bidang');
        
        // get data nominal_pengajuan, realisasi dan sisa dari post
        ...
        $this->load->view('kasir/monitoring_spj_konfirmasi',$data);
    }

    public function approvalTerimaSPJ() {
        $id_monitoring = $this->input->post('id_monitoring');
        $id_pengajuan_pemohon = $this->input->post('id_pengajuan_pemohon');
        //$nominal_umko_cair = $this->input->post('nominal_umko_cair');    
        ...
        // Update data monitoring
        $data = array(
            'kasir_spj_terima_dari' => $terima_dari,
            'kasir_spj_penerima' => addslashes($kasir_penerima),
            'kasir_spj_keterangan_disetujui' => addslashes($kasir_keterangan),
            'kasir_spj_tgl_disetujui' => $kasir_tgl_disetujui,
            'kasir_username' => $username,  // disetuijui oleh
            'kode_status' => '51', // Set status ke '31' untuk disetujui kasir
        );        
        //echo '<pre>';print_r($data);echo '<pre>'; exit();
        // update tabel monitoring
        $sql = $this->db->set($data)->where('id', $id_monitoring)->update('monitoring');
