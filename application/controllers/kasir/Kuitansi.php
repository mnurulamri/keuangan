...
class Kuitansi extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->library('Ajax_pagination_kuitansi');
        $this->perPage = 300;
    ...
    public function index() {
        ...
        $this->load->view('kasir/kuitansi-ajax-index', $data);        
        $this->load->view('template/footer');
    }

	public function menu()
	{		
		$sql = "SELECT * FROM menu where kasir = 1 order by parent, sort";
    ...
    public  function data()
    {
        ...
        if(!empty($params['search']['keywords'])){  // jika ada keyword pencarian 
			...
			$where = "WHERE kode_status IN (31, 40) AND nomor_pengajuan LIKE '%$keywords%'";
            ...
            # menghitung jumlah record
			$where = "WHERE kode_status IN (31, 40)";
            $sql_count = "SELECT id FROM monitoring $where";
        }
        ...
        $config['base_url']    = 'kasir/kuitansi';
        ...
        $this->ajax_pagination_kuitansi->initialize($config);
        ...
        # load the view
        $this->load->view('kasir/kuitansi-ajax-data', $data, false);
    }
    ...
    public function konfirmasiCetak()
    {
        ...
        $sql = "SELECT * FROM monitoring WHERE id IN (".implode(",", $id_monitoring).")";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        $data['kuitansi_data'] = $result;
        $data['id_monitoring'] = $id_monitoring;
        $this->load->view('kasir/kuitansi-modal-cetak', $data);

        // Kirim respons kembali ke AJAX
        //echo "Data dengan ID Kuitansi: " . implode(", ", $ids_kuitansi) . " telah diterima untuk proses cetak.";
    }
}