...
class Kuitansi_docx extends CI_Controller {
    ...
    private function get_kuitansi_data() 
    {
        ...
        $sql = "SELECT nomor_pengajuan, nominal_umko_cair, nominal_disetujui_umko, uraian FROM monitoring WHERE id IN ($id)";
        ...
        foreach ($result as $row) {
            $data_kuitansi[] = [
                ...
                'jumlah_uang' => $row['nominal_disetujui_umko'],
                'terbilang' => $this->terbilang($row['nominal_disetujui_umko']), // Anda bisa menambahkan fungsi untuk mengubah angka ke terbilang
                'untuk_pembayaran' => htmlspecialchars($row['uraian'], ENT_QUOTES, 'UTF-8') .' ' . $row['nomor_pengajuan'],
