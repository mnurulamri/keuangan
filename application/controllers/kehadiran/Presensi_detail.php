...
class Presensi_detail extends CI_Controller 
{
	...
	public function index()
	{
		...
		$this->load->view('kehadiran/presensi_detail', $data);
	}
	...
	public function dataPresensi()
	{
		...
		$tanggal1 = $tahun.'-'.substr($bulan, 0, 3).'01';
		$tanggal2 = $tahun.'-'.$bulan;
		...
		if($flag == 1){
			...
			$nip = $this->session->userdata['logged_in_presensi']['nip_presensi'];
	...
	function shift($array_shift, $tanggal1, $tanggal2){
		...
		if(!empty($array_shift)){
			$begin = new DateTime($tanggal1); //new DateTime( $array_shift['TglAwal'] ); 
