...
class Monitoring_ajax extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('yunior_akuntan/monitoring-ajax-index', $data);        
...
//print_r($kode_dpsj); exit();
        ...
        { // jika tidak ada keyword pencarian dan status adalah Semua
            $WHERE .= "AND a.kode_status >= 61"; // set default where clause for kode_status >= 41    
        } 
        else if(isset($keywords) && $keywords !== '' && $status == 'Semua')
        { // jika ada keyword pencarian dan status adalah Semua
            $WHERE .= "AND a.nomor_pengajuan LIKE '%$keywords%' AND a.kode_status >= 61"; // set default where clause for kode_status >= 41
        } 
        ...
        # load the view
        $this->load->view('yunior_akuntan/monitoring-ajax-data', $data, false);
