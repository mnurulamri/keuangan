...
class Monitoring extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('manajer/monitoring-ajax-index', $data);        
    ...
    public function konfirmasiApproval() {	
        ...
        $data['manajer_tgl_disetujui'] = $monitoring['anggaran_tgl_disetujui'] ?? null;
        $data['manajer_keterangan'] = $monitoring['manajer_keterangan'] ?? null;

        $this->load->view('manajer/monitoring_konfirmasi',$data);
    }

    public function approval() {
        $id_monitoring = $this->input->post('id_monitoring');
        $id_pengajuan_pemohon = $this->input->post('id_pengajuan_pemohon');
        $manajer_keterangan = $this->input->post('manajer_keterangan');
        $manajer_tgl_disetujui = date('Y-m-d H:i:s');
        ...
        $data = array(
            ...
            'manajer_keterangan_disetujui' => addslashes($manajer_keterangan),
            'manajer_tgl_disetujui' => $manajer_tgl_disetujui,
            'manajer_username' => $username,  // disetuijui oleh
    ...
    public function pending() {
        ...
        $id_pengajuan_pemohon = $this->input->post('id_pengajuan_pemohon');
        $manajer_keterangan = $this->input->post('manajer_keterangan');
        $manajer_tanggal = $this->hariTanggalToDb($this->input->post('manajer_tanggal'));
        $manajer_waktu = $this->input->post('manajer_waktu');
        $manajer_tgl_pending = $manajer_tanggal.' '.$manajer_waktu;
        $username = $this->session->userdata('logged_anggaran')['username'] ?? '';
        $form = $this->input->post('nama_form');
		$kode_status = '33';
        ...
        $data = array(
            //'nominal_disetujui_umko' => $nominal_disetujui_umko,
            'manajer_keterangan_pending' => addslashes($manajer_keterangan),
            'manajer_tgl_pending' => $manajer_tgl_pending,
            //'manajer_username' => $username,  // disetuijui oleh
            'kode_status' => $kode_status, // Set status ke '11' untuk disetujui unti anggaran
        );        
    ...
    public function batal() {
        ...
        $id_pengajuan_pemohon = $this->input->post('id_pengajuan_pemohon');
        $manajer_keterangan = $this->input->post('manajer_keterangan');
        $manajer_tgl_batal = date('Y-m-d H:i:s');
        $username = $this->session->userdata('logged_anggaran')['username'] ?? '';
        $form = $this->input->post('nama_form');
		$kode_status = '32';
        ...
        $data = array(
            //'nominal_disetujui_umko' => $nominal_disetujui_umko,
            'manajer_keterangan_pending' => addslashes($manajer_keterangan),
            'manajer_tgl_batal' => $manajer_tgl_batal,
            //'manajer_username' => $username,  // disetuijui oleh
            'kode_status' => $kode_status, // Set status ke '11' untuk disetujui unti anggaran
        );        
    ...
    public function detailApproval() {
        ...
        $query = $this->db->query($sql, array($id_pengajuan_pemohon));
        $monitoring = $query->row_array();
        $data['manajer_tgl_disetujui'] = $monitoring['manajer_tgl_disetujui'] ?? null;
        $data['manajer_keterangan'] = $monitoring['manajer_keterangan'] ?? null;

        $this->load->view('manajer/monitoring_detail',$data);
    }
