...
class Anggaran extends CI_Controller {
    ...
    public function search_project()
    {
		...
		/*$sql = "SELECT kode_kegiatan, nama_kegiatan 
                FROM view_anggaran_mutasi 
                WHERE kode_dpsj = '$dpsj' AND tahun_anggaran = '$tahun_anggaran' AND nama_kegiatan LIKE '%$kata%' AND flag_payroll IN ('Procost Unit', 'Procost Umum')
                GROUP BY nama_kegiatan LIMIT 10";*/

        // jika session logged_anggaran PUM_REMUN maka buka semua kegiatan, jika PUM_UNIT maka buka kegiatan berdasarkan dpsj
        if($this->session->userdata('logged_anggaran')['username'] == 'PUM_REMUN'){
            
            $sql = "SELECT kode_kegiatan, nama_kegiatan, kode_dpsj, deskripsi_dpsj 
                    FROM view_anggaran_mutasi 
                    WHERE tahun_anggaran = '$tahun_anggaran' AND nama_kegiatan LIKE '%$kata%'
                    ";    
        } else {
            $sql = "SELECT DISTINCT kode_kegiatan, nama_kegiatan, kode_dpsj, deskripsi_dpsj 
                    FROM (
                    SELECT kode_kegiatan, nama_kegiatan, kode_dpsj, deskripsi_dpsj 
                    FROM view_anggaran_mutasi 
                    WHERE tahun_anggaran = '$tahun_anggaran' 
                        AND nama_kegiatan LIKE '%$kata%' 
                        AND flag_payroll IN ('Procost Umum')
                    
                    UNION ALL
                    
                    SELECT kode_kegiatan, nama_kegiatan, kode_dpsj, deskripsi_dpsj 
                    FROM view_anggaran_mutasi 
                    WHERE kode_dpsj = '$dpsj' 
                        AND tahun_anggaran = '$tahun_anggaran' 
                        AND nama_kegiatan LIKE '%$kata%' 
                        AND flag_payroll IN ('Procost Unit')
                    ) AS gabungan
                    ";            
        }
		...
		if($array->num_rows()>0){
			...
			<table class="autocomplete-pc" id="test" >
				<tr>
					<th width="80px">KODE DPSJ</th>
					<th width="200px">NAMA DPSJ</th>
				...
				foreach ($array->result_array() as $row){
					$kotaksuggest.= '
					<tr>
						<td><div class="isi_pc" data-value="'.$row['nama_kegiatan'].'">'.$row['kode_dpsj'].'</div></td>
						<td><div class="isi_pc" data-value="'.$row['nama_kegiatan'].'">'.$row['deskripsi_dpsj'].'</div></td>
    ...
    public function search_akun() {
		...
		$sql = "SELECT kode_akun, deskripsi_akun, anggaran, kode_dana, kategori_kegiatan FROM view_anggaran_mutasi WHERE kode_kegiatan = '$kode_kegiatan' AND tahun_anggaran = '$tahun_anggaran' AND (deskripsi_akun LIKE '%$kata%' OR kode_akun LIKE '%$kata%') ";
		$array = $this->db->query($sql);
		//echo $sql; print_r($array_ruang->result_array()); 
		//$array_ruang = $ppf_web_db->select('*')->from('ruang_rapat')->where('nm_ruang')->limit('10')->get();
		...
		if($array->num_rows()>0){
			...
			<table class="autocomplete-pc" id="test" >
				<tr>
					...
					<th width="150px">DESKRIPSI AKUN</th>
                    <th width="10px">KODE DANA</th>
					<th width="150px">KATEGORI KEGIATAN</th>
				</tr>';
				foreach ($array->result_array() as $row){
					$kotaksuggest.= '
					<tr>
						<td><div class="isi_akun">'.$row['kode_akun'].'</div></td>
						<td><div class="isi_akun" data-value="'.$row['kode_akun'].'">'.$row['deskripsi_akun'].'</div></td>
						<td><div class="isi_akun">'.$row['kode_dana'].'</div></td>
						<td><div class="isi_akun">'.$row['kategori_kegiatan'].'</div></td>
					</tr>';
				}
