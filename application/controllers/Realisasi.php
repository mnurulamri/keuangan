...
class Realisasi extends CI_Controller {
    ...
    function ajukan_konfirmasi()
    {
        // ajukan realisasi per data pengajuan pemohon
        
        // get id_pengajuan_pemohon
        $id_pengajuan_pemohon = $this->input->post('id_pengajuan_pemohon');

        if (!$id_pengajuan_pemohon) {
            // If no id_pengajuan_pemohon is provided, redirect to the index page
            //redirect('realisasi');
        }

        $data['id_pengajuan_pemohon'] = $id_pengajuan_pemohon;

        # get rincian pengajuan_rincian berdasarkan id_pengajuan_pemohon
            $sql = "SELECT * FROM pengajuan_rincian WHERE id_pengajuan_pemohon in ($id_pengajuan_pemohon)";           
            ...
            $sql_realisasi = "SELECT id_pengajuan_rincian, SUM(bruto) as total_bruto, SUM(pph) as total_pph, SUM(netto) as total_netto 
                FROM realisasi WHERE id_pengajuan_rincian IN 
                    (SELECT id FROM pengajuan_rincian WHERE id_pengajuan_pemohon IN ($id_pengajuan_pemohon)) 
        ...
        // Load the form for creating a new realisasi
        $this->load->view('unit_kerja/realisasi_ajukan', $data);
    }
	...
	function ajukan(){
        ...
        $data = array(
            'tgl_pengajuan_realisasi' => date('Y-m-d'),
            'kode_status' => '44' // status menunggu verifikasi kasir
        );        
        //print_r($data); exit();
        // update tabel monitoring
        $sql = $this->db->set($data)->where('id_pengajuan_pemohon', $id_pengajuan_pemohon)->update('monitoring');

        // update kode_status pada tabel pengajuan_pemohon
        $data_pengajuan = array(
            'kode_status' => '44' //menunggu verifikasi kasir
        );
        $this->db->where('id', $id_pengajuan_pemohon);
        $this->db->update('pengajuan_pemohon', $data_pengajuan);
        
		echo 'data realisasi sudah diajukan';
	}
