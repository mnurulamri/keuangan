...
class Auth extends CI_Controller {
    ...
    {
        $this->load->view('auth/login_view_test');
    }
    ...
    public function set_role_test()
    {
		...
		if($query->num_rows() == 1) {
	        ...
	        if ($role == 'admin') {
                ...
                // set dpsj yg menempel pada username
				foreach($array_user as $row){
					$array_dpsj[] = $row['kode_dpsj'];
				}

                $sql = "SELECT kode_unit FROM unit_kerja WHERE kode_bidang = ?";
                $query = $this->db->query($sql, array($kode_bidang));
                $result = $query->result_array();
                foreach($result as $row){
                    $kode_unit = $row['kode_unit'];
                }
				//echo '<pre>';print_r($array_dpsj);echo '</pre>'; exit();
	            // set session dengan data user dan tampilkan pilihan role            
	            $newdata = array(
	                'role'  => $role,
	                'username'  => $username,
	                'kode_bidang'  => $kode_bidang,
					'array_dpsj'  => $array_dpsj,
                    'kode_unit' => $kode_unit
	            );
			...
			// jika jumlah row lebih dari satu
			if ($role == 'pum') {
				// set dpsj yg menempel pada username
				foreach($array_user as $row){
					$array_dpsj[] = $row['kode_dpsj'];
    ...
    public function set_role_bridge_pum_admin()
	{
        $kode_bidang = $this->input->post('kode_bidang');
		$username ='admin';
		$role = 'pum';
        // ambil data unit dari database unit_kerja
        $sql = "SELECT * FROM unit_kerja WHERE kode_bidang = '$kode_bidang'";
        $query = $this->db->query($sql);
        $array_unit = $query->result_array();

		// ambil data role dari database users
        $sql = "SELECT DISTINCT * FROM users  WHERE kode_bidang = '$kode_bidang'";
        $query = $this->db->query($sql);
        /*if ($query->num_rows() == 0) {
            $this->session->set_flashdata('error', 'Username tidak ditemukan!');
            redirect('auth/login');
