...
class Monitoring_ajax extends CI_Controller {
    ...
    public  function data()
    {
        ...
        $where = "WHERE kode_status IN (13,61,62,63,71) "; // set default where clause for kode_status >= 11
		if ($status == 'Diretur'){
			$where = "WHERE kode_status IN (62,63,64) AND nomor_pengajuan LIKE '%$keywords%'";
		} else if ($status == 'Disetujui'){
			$where = "WHERE kode_status IN (13,71) AND nomor_pengajuan LIKE '%$keywords%'";
		} else {
	        if(isset($keywords) && $keywords !== '' && isset($status) && $status !== '' && $status !== 'Semua') {  // jika ada keyword pencarian 		
				$where = "WHERE nomor_pengajuan LIKE '%$keywords%' AND kode_status = '$status'";
	        } elseif(isset($keywords) && $keywords !== '') {
	            $where = "WHERE nomor_pengajuan LIKE '%$keywords%'";
	        } elseif(isset($status) && $status !== '' && $status !== 'Semua') {
	            $where = "WHERE kode_status = '$status'";
	        } else {
	            $where = "WHERE kode_status >= 11"; // default condition if no search criteria
	        }
		}
        ...
        if(!empty($params['search']['sortBy'])) {
            ...
            $order_by = "ORDER BY nomor_pengajuan $order";
        } else {
            // Jika tidak ada parameter sortBy, urutkan data secara default DESC
            $order_by = 'ORDER BY nomor_pengajuan DESC';
        }
        ...
        $sql_monitoring = "SELECT * FROM monitoring $where ORDER BY SUBSTR(nomor_pengajuan,9,2) DESC, SUBSTR(nomor_pengajuan,1,3) DESC $limit";
        ...
        $config['base_url']    = 'korpum/monitoring_ajax';
        ...
        # load the view
        $this->load->view('korpum/monitoring-ajax-data', $data, false);
    }
}