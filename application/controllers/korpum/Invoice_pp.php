...
class Invoice_pp extends CI_Controller {
    ...
    public function index() 
    {
        ...
        $this->load->view('korpum/invoice-ajax-index', $data);        
    ...
    public function data(){
        ...
        $sql = "SELECT a.id, b.nomor_pengajuan, b.uraian, b.no_invoice_pp, b.no_tiket, b.tahun, b.bulan, b.tgl, a.kode_dpsj, a.deskripsi_dpsj, a.kode_kegiatan, a.kode_akun, a.kode_dana, a.aktual_report as aktual, a.pph, a.netto, b.id_pengajuan_pemohon as id_pengajuan_pemohon, invoice_status, form, pph_d02, netto_d02, netto_d01_d02
        ...
        $query = $this->db->query($sql, array($tahun, $bulan));
        $data['result'] = $query->result_array();
        $this->load->view('korpum/invoice-ajax-data', $data);

    }
    
	public function invoice_approval_form() {
        ...
        $this->load->view('korpum/korpum_invoice_approval_form', $data); 
    ...
    public function simpan_bayar() {
        ...
        $this->db->where('id_pengajuan_pemohon', $this->input->post('id_pengajuan_pemohon'));
        $this->db->update('monitoring', $data);
        
        // update tabel pengajuan_pemohon, set kode_status = 76 
        $data = array(
            'kode_status' => $this->input->post('kode_status')
        );
    ...
    public function simpan_retur() {
        ...
        // update tabel pengajuan_pemohon, set kode_status berdasarkan input kode_status
        $data = array(
            'kode_status' => $this->input->post('kode_status')
        );
    ...
    public function get_rincian(){
        // get post id_pengajuan_pemohon
        $id_pengajuan_pemohon = $this->input->post('id_pengajuan_pemohon');
        
        // ambil rincian berdasarkan id pengajuan pemohon
        $sql_rincian = "SELECT * FROM view_pengajuan_rincian_realisasi WHERE id_pengajuan_pemohon = '$id_pengajuan_pemohon' ";           
        $query_rincian = $this->db->query($sql_rincian);
        $result_rincian = $query_rincian->result_array();
        if($query_rincian->num_rows() > 0) {
            $array_rincian = array();
        ...
        $this->load->view('anggaran/invoice-detail-pengajuan-rincian', array('array_rincian'=>$array_rincian));  
    }
    ...
    // Di dalam class Invoice extends CI_Controller
    
    // 1. Menampilkan form pencarian pengajuan yang belum masuk ke invoice (mirip search_pengajuan)
    public function tambah_form_data() {
        $data['no_tiket'] = $this->input->post('no_tiket');
        // Ambil data pengajuan yang belum ada di invoice
        // Gunakan query builder atau query manual sesuai kebutuhan
        $data['pengajuan_tersedia'] = $this->db->query("SELECT * FROM view_pengajuan WHERE status = 'terima' AND id NOT IN (SELECT id_pengajuan_pemohon FROM invoice_rekap_procost)")->result_array();
        
        $this->load->view('invoice/tambah_form_konfirmasi', $data);
    }
    
    // 2. Konfirmasi data sebelum simpan (menggunakan view yang sudah ada atau baru)
    public function tambah_konfirmasi() {
        $id_pengajuan = $this->input->post('id_pengajuan'); // Array ID
        $no_tiket = $this->input->post('no_tiket');
        
        // Ambil detail data dari header untuk kebutuhan insert
        $header = $this->db->where('no_tiket', $no_tiket)->get('invoice_rekap_procost')->row();
        
        // Query rincian pengajuan yang dipilih
        $ids = implode(',', $id_pengajuan);
        $data['result'] = $this->db->query("SELECT * FROM view_pengajuan_rincian_realisasi WHERE id_pengajuan_pemohon IN ($ids)")->result_array();
        $data['header'] = $header;
        
        $this->load->view('anggaran/invoice-tambah-konfirmasi', $data);
    }
    
    // 3. Simpan ke database
    public function tambah_simpan_final() {
        $no_tiket = $this->input->post('no_tiket');
        $id_pengajuan = $this->input->post('id_pengajuan'); // array
        $header = $this->db->where('no_tiket', $no_tiket)->get('invoice_rekap_procost')->row();
        
        foreach ($id_pengajuan as $id) {
            $p = $this->db->where('id', $id)->get('view_pengajuan')->row(); // Ambil nomor_pengajuan
            $data = array(
                'no_tiket' => $no_tiket,
                'no_invoice_pp' => $header->no_invoice_pp,
            ...
            // Update monitoring
            $this->db->where('id_pengajuan_pemohon', $id)->update('monitoring', ['no_invoice_pp' => $header->no_invoice_pp]);
        }
        echo "success";
    }
    
    // Di dalam class Invoice
    
    
    public function simpan_tambahan_invoice() {
        // Tangkap input dari form konfirmasi
        $id_pengajuan = $this->input->post('id_pengajuan_pemohon'); // Array
        $no_tiket = $this->input->post('no_tiket');
        
        // Ambil detail header dari tiket yang sudah ada agar data konsisten
        $header = $this->db->where('no_tiket', $no_tiket)->get('invoice_rekap_procost')->row();
        ...
        foreach ($id_pengajuan as $id) {
            $data = array(
                ...
                'tgl' => $header->tgl,
                'id_pengajuan_pemohon' => $id,
                'nomor_pengajuan' => $this->db->get_where('view_pengajuan', ['id' => $id])->row()->nomor_pengajuan
            );
            $this->db->insert('invoice_rekap_procost', $data);
        }
        
        // Redirect kembali ke index
        $this->session->set_flashdata('message', 'Data berhasil ditambahkan ke tiket!');
