...
class Rekap_TotalBiaya extends CI_Controller {
    ...
    public function index (){	
        ...
        $sql = "SELECT kode_akun, deskripsi_akun, keterangan, sum(bruto) as bruto, sum(pph) as pph, sum(netto) as netto 
                FROM realisasi 
                WHERE id_pengajuan_pemohon = ? 
                GROUP BY id_pengajuan_pemohon, kode_akun";
        $query = $this->db->query($sql, array($id));
        $result = $query->result_array();
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('A6', 'REKAP TOTAL BIAYA');
        ...
        $columns_header_1 = ['A7'=>'NO', 'B7'=>'NOMOR DAN NAMA AKUN', 'D7'=>'KETERANGAN', 'E7'=>'BRUTO', 'F7'=>'PPh (RP)', 'G7'=>'NETTO'];
        ...
        foreach ($result as $item) {  
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$row, (int)$item['bruto'], PHPExcel_Cell_DataType::TYPE_NUMERIC)->getStyle('E'.$row)->getNumberFormat()->setFormatCode("#,##")->applyFromArray($style_row_middle);               
	        $objPHPExcel->getActiveSheet()->getStyle('F'.$row)->applyFromArray($style_row);

            if($item['pph'] == 0){
                $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$row, (int)$item['pph'], PHPExcel_Cell_DataType::TYPE_NUMERIC)->getStyle('F'.$row)->applyFromArray($style_row_middle); 
                $objPHPExcel->getActiveSheet()->getStyle('F'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            } else {
                $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$row, (int)$item['pph'], PHPExcel_Cell_DataType::TYPE_NUMERIC)->getStyle('F'.$row)->getNumberFormat()->setFormatCode("#,##")->applyFromArray($style_row_middle); 
            }       
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('G'.$row, (int)$item['netto'], PHPExcel_Cell_DataType::TYPE_NUMERIC)->getStyle('G'.$row)->getNumberFormat()->setFormatCode("#,##")->applyFromArray($style_row_middle); 
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$row, $total_bruto, PHPExcel_Cell_DataType::TYPE_NUMERIC)->getStyle('E'.$row)->getNumberFormat()->setFormatCode("#,##")->applyFromArray($style_total);
           
        // total pph
        $objPHPExcel->getActiveSheet()->getStyle('F'.$row)->applyFromArray($style_total);
        if($item['pph'] == 0){
            $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$row, $total_pph, PHPExcel_Cell_DataType::TYPE_NUMERIC)->getStyle('F'.$row)->applyFromArray($style_total);
        } else {
            $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$row, $total_pph, PHPExcel_Cell_DataType::TYPE_NUMERIC)->getStyle('F'.$row)->getNumberFormat()->setFormatCode("#,##")->applyFromArray($style_total);
        }        
           
        // total netto
        $objPHPExcel->getActiveSheet()->getStyle('G'.$row)->applyFromArray($style_total);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('G'.$row, $total_netto, PHPExcel_Cell_DataType::TYPE_NUMERIC)->getStyle('G'.$row)->getNumberFormat()->setFormatCode("#,##")->applyFromArray($style_total);
        ...
        header('Content-Disposition: attachment;filename="RekapTotalBiaya_' . $nomor_pengajuan . '.xls"');
