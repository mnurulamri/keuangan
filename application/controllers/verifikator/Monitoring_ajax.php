...
//print_r($kode_dpsj); exit();
		...
		if($status == 'Menunggu Pemeriksaan Verifikator'){
          $where .= " AND kode_status IN (51,62) AND nomor_pengajuan LIKE '%$keywords%' AND SUBSTR(nomor_pengajuan, 12, 4) = '$tahun' AND SUBSTR(nomor_pengajuan, 9, 2) = '$bulan'";
        } else if($status == '51'){
          $where .= " AND kode_status IN (51,62) AND nomor_pengajuan LIKE '%$keywords%' AND SUBSTR(nomor_pengajuan, 12, 4) = '$tahun' AND SUBSTR(nomor_pengajuan, 9, 2) = '$bulan'";
        }
        else if ($status == 'Diretur'){
			$where .= " AND kode_status IN (12,22,32,33,42,52,63) AND nomor_pengajuan LIKE '%$keywords%' AND SUBSTR(nomor_pengajuan, 12, 4) = '$tahun' AND SUBSTR(nomor_pengajuan, 9, 2) = '$bulan'";
		} else if ($status == 'Disetujui'){
			$where .= " AND kode_status IN (13,21,23,61,71) AND nomor_pengajuan LIKE '%$keywords%' AND SUBSTR(nomor_pengajuan, 12, 4) = '$tahun' AND SUBSTR(nomor_pengajuan, 9, 2) = '$bulan'";
		} else {
