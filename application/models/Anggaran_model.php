...
class Anggaran_model extends CI_Model {
    ...
    public function generate_nomor_pengajuan($kode_unit, $tahun, $bulan, $nama_form) {
        $current_month = $bulan;
        $current_year = $tahun;
        ...
        $query = $this->db->get('view_search_pengajuan');
        $result = $query->row();
		...
		// jika D01 maka ANG jika D02 maka KEU
		if($nama_form == 'D01'){
			$kode_form = '/ANG.';
		} else {
			$kode_form = '/KEU.';
        ...
        return $nomor_urut . $kode_form . $bulan . '/' . $tahun . '-' . $kode_unit;
    }
