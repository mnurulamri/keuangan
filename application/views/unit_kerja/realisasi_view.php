...
<div class="containerx">
    ...
    <table  style="margin:auto; width:80%" class="table table-bordered table-stripedx">
        <tr>
            ...
            <td>:</td>
            <td width="70%" id="kegiatan" contenteditable="false"><?php echo $kegiatan;?></td>
        </tr>
    ...
    <div class="kotakx">
        <table class="table" style="margin:auto" id="tabel-rincian">
            ...
            <thead>
                <tr>
                    <td class="text-center text-warning" colspan="11"><b>REKAP BIAYA</b></td>
                </tr>
                <tr>
                    <th rowspan="2">NO</th>
            ...
            <tbody>
                ...
                if (isset($result_realisasi) && is_array($result_realisasi)) {
                    ...
                    foreach ($result_realisasi as $row) {
                        ...
                        echo '<td class="tanggal" >' . dbToTanggal($row['tanggal']) . '</td>';
                        echo '<td class="keterangan text-left" >' . $row['keterangan'] . '</td>';
                        echo '<td class="volume text-center" >' . $row['volume'] . '</td>';
                        echo '<td class="ket_volume text-center" >' . $row['ket_volume'] . '</td>';
                        echo '<td class="harga text-right" >' . number_format($row['harga'], 0, ',', '.') . '</td>';
                        echo '<td class="bruto text-right" >' . number_format($row['bruto'], 0, ',', '.') . '</td>';
                        echo '<td class="tarif_pajak text-center" >' . $row['persen_pajak'] . '</td>';
                        echo '<td class="pph text-right" >' . number_format($row['pph'], 0, ',', '.') . '</td>';
                        echo '<td class="netto text-right" >' . number_format($row['netto'], 0, ',', '.') . '</td>';
                        echo '<td class="catatan-perbaikan text-right">'.$row['catatan_perbaikan'].'</td>';
                        echo '</tr>';
                        $no++;
            ...
            <tfoot>
                <tr>
                    ...
                    <td class="total text-right"><?=number_format($bruto_total)?></td>
                    <td></td>
                    <td></td>
                    <td class="text-right"><?=number_format($netto_total)?></td>
                </tr>
            </tfoot>
        </table>        
    </div>
...
table#tabel-rincian thead tr th {
