...
<div class="containerx">
    ...
    <table  style="margin:auto; width:80%" class="table table-bordered table-stripedx">
        <tr>
            ...
            <td>:</td>
            <td width="70%" id="kegiatan" contenteditable="true"><?php echo $keterangan; ?></td>
        </tr>
        <tr style="display:none">
            <td class="label-1">HARI/TANGGAL</td>
            <td>:</td>
            <td id="jadwal" contenteditable="true"><?= date('Y-m-d') ?></td>
        </tr>
        <!--<tr>
    ...
    <div class="kotakx">
        <table style="margin:auto" id="tabel-rincian" class="table">
            <label for="">&nbsp;</label>
            <thead>
                <tr>
                    <td class="text-center" colspan="6" style="border-top:1px solid #fff;border-left:1px solid #fff; "></td>
					<td class="text-center" colspan="4" style="padding-top:1px; padding-bottom:1px; background: #F43378; color:#fff; font-weight:bold">UMKO: <?php echo number_format($komitmen)?></td>
                </tr>
                <tr>
                    <td class="text-center" colspan="10"><b>REKAP BIAYA</b></td>
                </tr>
                <tr style="background:#F2F0EF">
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="9" style="border: 1px solid #fff">
                        ...
                        <button class="btn btn-warning btn-sm" id="simpan" data-id="<?=$id_pengajuan_rincian?>" data-id_pengajuan_pemohon="<?=$id_pengajuan_pemohon?>"><i class="fa fa-floppy-o"></i> Simpan </button>                                                     
                        <button class="btn btn-default btn-sm rincian-biaya-excel" data-id="<?=$id_pengajuan_rincian?>" data-nomor-pengajuan="<?=$nomor_pengajuan?>" data-id_pengajuan_pemohon="<?=$id_pengajuan_pemohon?>"><i class="fa fa-file-excel-o text-default" style="font-size:16px;color:green;"></i> Cetak Excel</button>
                        <i class="pesan"></i>
                    </td>
                </tr>
            </tfoot>
...
$(document).ready(function()
{
	...
	$(document).on("click", ".bruto", function(evt){
		$(this).select();
        if (window.getSelection && document.createRange) {
            var range = document.createRange();
            range.selectNodeContents(this);
	...
	$(document).on("blur", ".bruto", function(evt){
		let keyCode = $(this).text()
        let value = keyCode.replace(/[^\d.]/g, '');
        //harga = formatNumberWithThousandSeparator(value); //
        let bruto = value.replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        $(this).text(bruto);
	});
	...
	$(document).on("click", ".pph", function(evt){
		$(this).select();
        if (window.getSelection && document.createRange) {
            var range = document.createRange();
            range.selectNodeContents(this);
