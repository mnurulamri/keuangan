    ...
    <table class="styled-table" id="tabel">
        ...
        <tbody>
            ...
            foreach($result as $row){
                ...
                $procost = $row['kode_kegiatan'].' - '.explode(":", $row['nama_kegiatan'])[1];
                ...
                <tr class="item">
                    ...
                    <td class="text-right detail_komitmen_aktual" '.$style_komitmen.' data-kode_kegiatan="'.$row['kode_kegiatan'].'" data-kode_akun="'.$row['kode_akun'].'" data-kode_dana="'.$row['kode_dana'].'" '.$toggle_komitmen.'>'.number_format($row['komitmen']).'</td>
                    <td class="text-right detail_komitmen_aktual" '.$style_aktual.' data-kode_kegiatan="'.$row['kode_kegiatan'].'" data-kode_akun="'.$row['kode_akun'].'" data-kode_dana="'.$row['kode_dana'].'" '.$toggle_aktual.'>'.number_format($row['aktual']).'</td>
        ...
        <tfoot>
            <tr style="font-weight: bold; background-color: #f8f9fa; color:#666">
                <td colspan="5" style="text-align: right;">TOTAL:</td>
                <td class="text-right" id="total-anggaran">0</td>
                <td class="text-right" id="total-komitmen">0</td>
                <td class="text-right" id="total-aktual">0</td>
                <td class="text-right" id="total-mutasi">0</td>
                <td class="text-right" id="total-sisa-anggaran">0</td>
            </tr>
        </tfoot>
    </table>
</div>
...
var input_kode_dpsj = document.getElementById("kode_dpsj-search");
...
var input_pagu = document.getElementById("pagu-search");
...
function search() {
    ...
    for (let i = 0; i < tr.length; i++) {
        ...
        let td = tr[i].cells;
        let td_kode_dpsj = td[0].innerText;
        let td_kode_kegiatan = td[1].innerText;
        let td_kode_akun = td[2].innerText;
        let td_pagu = td[4].innerText;
        
        if (td_kode_dpsj.toUpperCase().indexOf(filter_kode_dpsj) > -1 && 
            td_kode_kegiatan.toUpperCase().indexOf(filter_kode_kegiatan) > -1 && 
            td_kode_akun.toUpperCase().indexOf(filter_kode_akun) > -1 && 
            td_pagu.toUpperCase().indexOf(filter_pagu) > -1) {
            
            tr[i].style.display = "";
            
            // Hanya tambahkan total jika baris ditampilkan
            // Ambil nilai dari kolom KOMITMEN, AKTUAL, MUTASI, dan SISA ANGGARAN
            let mutasiText = td[8].innerText.replace(/,/g, '');
            let sisaAnggaranText = td[9].innerText.replace(/,/g, '');
            let AnggaranText = td[5].innerText.replace(/,/g, '');
