        ...
        <div class="col-sm-12 kotak">
            ...
            <div id="approvalForm" class="text-center">
                ...
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                        <label for="kasir_keterangan_retur_pengajuan" class="col-xs-4 text-right" style="color:#555">Catatan :</label>
                        <div class="col-xs-8">
                            <textarea class="form-control" id="kasir_keterangan_retur_pengajuan" name="kasir_keterangan_retur_pengajuan" rows="3"><?=$kasir_keterangan?></textarea>
                        </div>
                    </div> 
                </form>
                <!--<button type="button" class="btn btn-warning" id="btnApprovalSave">Pending</button>-->
                <button class="btn btn-success" id="setujui_pengajuan" data-id_pengajuan_pemohon="<?=$id_pengajuan_pemohon?>"  data-id_monitoring="<?=$id_monitoring?>"><i class="fa fa-check"></i> Lanjut Proses </button>
                <!--<button class="btn btn-warning" id="retur_pengajuan" data-id_pengajuan_pemohon="<?=$id_pengajuan_pemohon?>"  data-id_monitoring="<?=$id_monitoring?>"><i class="fa fa-undo"></i> Dikembalikan </button>-->
                <button class="btn btn-danger" id="batal_pengajuan" data-id_pengajuan_pemohon="<?=$id_pengajuan_pemohon?>"  data-id_monitoring="<?=$id_monitoring?>"><i class="fa fa-minus-circle"></i> Dibatalkan </button>
            </div>
        </div>
    </div>
...
{
    $('#setujui_pengajuan').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(res) {
				...
				$("#modal-approval").modal('hide');
    				$("#status_"+id_monitoring).text("Proses Pengisian SPJ");
    				$('#btn_pengembalian_'+id_monitoring).prop("disabled", true);
    ...
    $('#retur_pengajuan').on('click', function() {       

		if (!confirm("Apakah Anda yakin akan meretur data pengajuan ini?")) {
			return false;
		} else {
            var id_monitoring = $(this).data('id_monitoring');
            var id_pengajuan_pemohon = '<?=$id_pengajuan_pemohon?>';
            var kasir_keterangan = $('#kasir_keterangan_retur_pengajuan').val();
            var page = $("temp_page").val(); // halaman saat ini, bisa diubah sesuai kebutuhan
            var kasir_tanggal = $('#kasir_tanggal').val();
            var kasir_waktu = $('#kasir_waktu').val();

            $.ajax({
                url: '<?=base_url("kasir/monitoring/retur_pengajuan")?>',
                type: 'POST',
                data: {
                    id_monitoring: id_monitoring,
                    id_pengajuan_pemohon: id_pengajuan_pemohon,
                ...
                success: function(res) {
                    //getDataPageMonitoring(page); // refresh data
                    $("#modal-approval").modal('hide');
    				$("#status_"+id_monitoring).text("Retur Pengajuan");
    				$('#btn_pengembalian_'+id_monitoring).prop("disabled", true);
                    //location.href = "<?=base_url()?>kasir/kuitansi";
                    console.log(res);
                },
            ...
            // pencatatan kendali dokumen
            kendali_dokumen(flag_approve='Retur', id_monitoring, kasir_tanggal, kasir_waktu);
        }
    });

    $('#batal_pengajuan').on('click', function() {       
		...
		if (!confirm("Apakah Anda yakin akan membatalkan data pengajuan ini?")) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    //getDataPageMonitoring(page); // refresh data
                    $("#modal-approval").modal('hide');
    				$("#status_"+id_monitoring).text("Dibatalkan");
    				$('#btn_pengembalian_'+id_monitoring).prop("disabled", true);
                    //location.href = "<?=base_url()?>kasir/kuitansi";
                    /*if(res.status == 'success') {
                        alert('Approval berhasil disimpan.');
...
function kendali_dokumen(flag_approve, id_monitoring, kasir_tanggal, kasir_waktu){
    ...
    var kode_status = '';
    if(flag_approve == 'Retur'){
        kode_status = '40';
    } else if(flag_approve == 'Proses'){
        kode_status = '41';
