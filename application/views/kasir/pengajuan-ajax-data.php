...
foreach($array_monitoring as $key => $value)
{
    ...
    $html.= '
        <td>'.$posts[$key]['anggaran_tgl_disetujui'].'</td>
        ...
        <td>';
        if($posts[$key]['kode_status'] == 44){
            // jika nama_form adalah D02, maka jangan tampilkan tombol Periksa SPJ
            if($posts[$key]['form'] != 'D02') {
                $html.= '
                <!--<button class="btn btn-primary btn-xs terima-spj" data-id_monitoring="'.$posts[$key]['id'].'" data-id_pengajuan_pemohon="'.$posts[$key]['id_pengajuan_pemohon'].'" data-toggle="modal" data-target="#modal-catatan">Terima SPJ</button>-->
                <button class="btn btn-primary btn-xs lanjut-proses" data-id_monitoring="'.$posts[$key]['id'].'" data-id_pengajuan_pemohon="'.$posts[$key]['id_pengajuan_pemohon'].'" data-toggle="modal" data-target="#modal-catatan">Terima SPJ</button>';
    ...
    <tr id="'.$toggleId.'" style="display:none;">
        <td colspan="11" style="text-align:center">
            <table id="tabel" style="margin:auto; width:100%; border-collapse:collapse; border:1px solid #ddd; margin-top:10px;">
                ...
                foreach($value as $row) {
                    ...
                    /*if($array_realisasi[$row['id']]['total_netto'] == 0) {
                        $html.= '<td style="border-right:1px solid #ddd;"><button class="btn btn-primary btn-xs buat-realisasi" data-nomor_pengajuan="'.$key.'" data-id="'.$row['id'].'" data-nominal_pengajuan="'.$posts[$key]['nominal_pengajuan'].'" data-jumlah_umko="'.$row['komitmen'].'" data-toggle="modal" data-target="#modal-realisasi">buat</button></td>';
                        ...
                        $html.= '<td style="border-right:1px solid #ddd;"><button class="btn btn-success btn-xs edit-realisasi" data-nomor_pengajuan="'.$key.'" data-id="'.$row['id'].'" data-nominal_pengajuan="'.$posts[$key]['nominal_pengajuan'].'" data-jumlah_umko="'.$row['komitmen'].'" data-toggle="modal" data-target="#modal-realisasi">edit</button></td>';
                    }
...
echo $this->ajax_pagination_verifikator->create_links();
?>
