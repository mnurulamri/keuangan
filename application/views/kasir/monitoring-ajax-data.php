...
<table class="styled-table" width="100%">
    ...
    <tbody>
        <?php if (!empty($posts)): ?>
            ...
            ?>
                <tr id="<?=$toggleHeadId?>">
                    <td onclick="toggleDetail('<?=$toggleId?>', '<?=$toggleHeadId?>', '<?=$barisKe?>')" style="cursor:pointer; <?=$text_decoration?>" id="<?=$toggleHeadId?>"><?= (isset($row['anggaran_tgl_disetujui'])|| !empty($row['anggaran_tgl_disetujui']) || is_null($row['anggaran_tgl_disetujui'])) ? dateTimeToTanggal($row['anggaran_tgl_disetujui']) : '' ?><?=$toggleId?></td>
                    ...
                    <td onclick="toggleDetail('<?=$toggleId?>', '<?=$toggleHeadId?>', '<?=$barisKe?>')" style="cursor:pointer; <?=$text_decoration?>" id="<?=$toggleHeadId?>" class="nominal_pengajuan"><?= isset($row['nominal_pengajuan']) ? number_format($row['nominal_pengajuan']) : '' ?></td>
                    <td onclick="toggleDetail('<?=$toggleId?>', '<?=$toggleHeadId?>', '<?=$barisKe?>')" style="cursor:pointer; <?=$text_decoration?>" id="<?=$toggleHeadId?>" class="realisasi"><?= isset($subtotal_realisasi_main) ? number_format($subtotal_realisasi_main) : '0' ?></td>
                    <td onclick="toggleDetail('<?=$toggleId?>', '<?=$toggleHeadId?>', '<?=$barisKe?>')" style="cursor:pointer; <?=$text_decoration?>" id="<?=$toggleHeadId?>" class="sisa"><?= isset($subtotal_sisa_main) ? number_format($subtotal_sisa_main) : '0' ?></td>
                    <!--
                    <td>
                    ...
                    <td>
                        <?php
                         if($row['kode_status'] == 31){
                            echo '<button id="btn_pengembalian_'.$row['id'].'" class="btn btn-primary btn-xs approval" data-id_pengajuan_pemohon="'.$row['id_pengajuan_pemohon'].'" data-id_monitoring="'.$row['id'].'" data-kode_dpsj="'.$row['kode_dpsj'].'" data-toggle="modal" data-target="#modal-approval" >Pengambilan UMKO</button>';
                             
                         } else if ($row['kode_status'] == 44){
                            echo '<button id="btn_terima_spj_'.$row['id'].'" class="btn btn-success btn-xs terima-spj" data-id_pengajuan_pemohon="'.$row['id_pengajuan_pemohon'].'" data-id_monitoring="'.$row['id'].'" data-kode_dpsj="'.$row['kode_dpsj'].'" data-toggle="modal" data-target="#modal-approval" >Pengembalian UMKO</button>';
